...
// Implements debug logging for numbers by converting them into strings
...
// C printf using the above MicroVsnprintf

extern "C" int puts(const char *s);
extern "C" int printf(const char *fmt, ...)
{
  const static int max_log_len = 256;
  char log_buffer[max_log_len];

  va_list args;
  va_start(args, fmt);
  int bytes_written = MicroVsnprintf(log_buffer, max_log_len, fmt, args);
  va_end(args);
